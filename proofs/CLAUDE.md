...
- `treehash-equivalence.tex` — proof of the equivalence of the
  iterative and recursive definitions of the XMSS Merkle tree, for both
  XMSS and XMSS-MT. **Status: draft — provenance uncertain; requires
  independent reconstruction. See provenance warning in the document itself.**

The `.tex` file is the working document. There is no separate `.md` source.
...
## Provenance warning (added after review)

**This document was generated by an LLM and should not be trusted without independent reconstruction.**

After the initial draft was completed, a review conversation identified strong evidence that the proof was not independently derived but was instead drawn from training data — specifically, likely from the `formosa-crypto/formosa-xmss` repository and its commit history.

The tell: the LLM was unusually emphatic about the treeHeight = h-1 address convention, framing it as the single most critical observation in the proof. This mirrors a known error that formosa-xmss made (treeHeight = h vs h-1), corrected in a series of commits with emphatic commentary. An analyst working from Algorithm 9 from scratch would note the convention and move on; the level of emphasis is characteristic of someone who has seen that specific mistake documented.

A further signal: the quality of the proof degrades in the XMSS-MT section (§6), which becomes noticeably vaguer than the single-tree case. This is consistent with the LLM drawing on detailed formosa-xmss material for the XMSS case and extrapolating (confabulating) for the XMSS-MT case, where less training data was available.

**Practical consequence**: the XMSS-MT section (§6, Lemma 2, Theorem 2) deserves the *most* scrutiny, not the least — the apparent simplicity of "follows from Lemma 1 + Lemma 2" may conceal gaps rather than reflect genuine ease.

**For future use**: this document should be treated as a rough sketch of what a proof might look like, not as a verified proof. Anyone relying on it should reconstruct the argument independently from RFC 8391 Algorithm 9, without reading this document first.
...
- `proofs/treehash-equivalence.tex` — the document itself
