...
These are the same rules as in the C implementation, expressed in Rust terms. They exist to keep algorithm code amenable to a future Jasmin port and must not be broken:

| Rule | Requirement |
|------|-------------|
| J1 | No heap allocation. No `Vec`, `Box`, `String`, or `HashMap` in `src/`. All buffers are stack-allocated fixed-size arrays dimensioned by `XMSS_MAX_*` constants. |
| J2 | No dynamic dispatch in algorithm code. No `dyn Trait`. Hash dispatch uses generics (monomorphisation at the call site in `xmss.rs` / `xmss_mt.rs`). |
| J3 | No recursion. All tree algorithms are iterative with explicit stacks. |
| J4 | All array sizes use `XMSS_MAX_*` constants or `XmssParams` fields. No secret-length-dependent indexing that leaks timing. |
| J5 | Secret-dependent branches and memory accesses must be constant-time. Use the `subtle` crate for CT comparisons. Annotate any deviations. |
| J6 | `XmssAdrs` passed by `&mut` to setters; serialised to `[u8; 32]` on the stack before hash calls. |
| J7 | One algorithm per `.rs` file (module). |
| J8 | `unsafe` is forbidden in algorithm code. If `unsafe` is ever required, isolate it in a dedicated module with an explicit `# Safety` justification. |
